...
/* Shared physician-facing rendering of one session's history: demographics,
   the red-flag reason together with the patient's own words that raised it,
   the draft summary (every field editable), the AYUSH panel, the assembled
   FHIR bundle, and sign-off. Used by both app/physician/[id]/page.tsx and
   app/pt_history/page.tsx so the two views cannot drift apart. Patient text
   is always rendered as plain React children, never via
   dangerouslySetInnerHTML. */

import { useEffect, useState } from "react";
import AyushPanel from "./AyushPanel.tsx";
...
export default function PatientHistory({
  ...
  if (error && !session) {
    return <p className="text-red-700 bg-red-50 rounded-lg px-3 py-2">{error}</p>;
  }
